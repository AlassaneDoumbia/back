    ...
    def parametre(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if roaming == "out":
                ...
                sql = "select id, Date as date, GTPv2_C_Attempts_OUT as Total_Transactions, GTPv2_C_Average_Session_Duration_msec_OUT as heure, Efficacité_OUT as EFF from bearer_bearer_out where Opérateur='"+country_operator+"' AND date_mns BETWEEN '"+str(debut_obj)+"' AND '"+str(fin_obj)+"' group by date  order by date limit 30 "
                ...
                sql = "select id, Date as date, GTPv2_C_Attempts_IN as Total_Transactions, GTPv2_C_Average_Session_Duration_msec_IN as heure, Efficacité_IN as EFF  from bearer_bearer_in where Opérateur='"+country_operator+"' AND date_mns BETWEEN '"+str(debut_obj)+"' AND '"+str(fin_obj)+"' group by date order by date limit 30 "
                queryset = Params.objects.raw(sql)
                razbi = customSerializer(queryset, many=True)
