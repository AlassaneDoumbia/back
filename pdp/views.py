    ...
    def parametre(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if roaming == "out":
                ...
                sql = "select id, Date as date, GTP_C_Procedure_Attempts as Total_Transactions, GTP_C_Procedure_Average_Latency_msec as heure, Eff_PDP_Act as EFF from pdp_pdp_out  where date_mns BETWEEN '"+str(debut_obj)+"' AND '"+str(fin_obj)+"' group by date order by date limit 30;"
                ...
                sql = "select id, Date as date, GTP_C_Procedure_Attempts_IN as Total_Transactions, GTP_C_Procedure_Average_Latency_msec_IN as heure, Eff_PDP_Act_IN as EFF from pdp_pdp_in where date_mns BETWEEN '"+str(debut_obj)+"' AND '"+str(fin_obj)+"' group by date order by date limit 30;"
                queryset = Params.objects.raw(sql)
                razbi = customSerializer(queryset, many=True)
